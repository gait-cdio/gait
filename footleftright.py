...
def track_direction(track):
    x = track[:,0]
    slope, dontcare = np.polyfit(range(len(x)), x, 1)  # weights here could be useful if start and stop mess it up
    if slope > 0:
...
def left_foot_right_foot(track_input):
    ...
    return [ordered_feet_with_toe_heel[1],ordered_feet_with_toe_heel[0]], dir
